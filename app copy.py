...
def ocean_extremes():
    response = requests.get('https://psl.noaa.gov/gcos_wgsp/Timeseries/Data/nino34.long.anom.data')
    ...
    for line in raw_data:
        ...
        if len(split_line) == 13:
            ...
            for month in range(1, 13):
                if float(split_line[month]) != -99.99:
                    data[f"{year}-{month:02d}"] = float(split_line[month])
    return render_template('ocean_extremes.html', data=data)
